...
def menu_clients():
    while True:
        ...
        if choice == "1":
            ...
            for c in clients:
                print(
                    f"ID: {c.id} | {c.name} | Тел: {c.phone} | Статус: {c.reliability}")
            ...
            client = Client(name=name, passport=passport,
                            phone=phone, reliability="Высокая")
            client.save()
            ...
            passport = input(
                f"Новые паспортные данные [{current_client.passport}]: ")
            phone = input(f"Новый номер телефона [{current_client.phone}]: ")
            reliability = input(
                f"Новый статус благонадежности [{current_client.reliability}]: ")
